...
namespace Basketball.Common.ModelMetaData
{
    public class ConventionalModelMetadataProvider : DataAnnotationsModelMetadataProvider
    {
        ...
        protected override ModelMetadata CreateMetadata(IEnumerable<Attribute> attributes, Type containerType, Func<object> modelAccessor, Type modelType, string propertyName)
        {
            ...
            // HACK!
            // The above bit of code doesn't work. For some reason metadata.DisplayName == metadata.PropertyName is never true
            ...
            if (!string.IsNullOrEmpty(metadata.PropertyName))
            {
                ...
                displayName = metadata.PropertyName.Replace("Id", "").SplitUpperCaseToString();

                if (attributes.Where(x => x is RequiredAttribute).Any())
                    displayName += " *";

                // If metadata.DisplayName is null then bad things happen
